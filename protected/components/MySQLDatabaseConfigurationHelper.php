...
class MySQLDatabaseConfigurationHelper implements DatabaseConfigurationHelper {
	...
	public function createDatabase($databaseConfig) {
		$conn = new MySQLi($databaseConfig['server'], $databaseConfig['username'], $databaseConfig['password']);
		$dbcheck=$this->requireDatabaseOrCreatePermissions($databaseConfig);
		if(!$dbcheck['alreadyExists']){
			$conn->query("CREATE DATABASE ".$databaseConfig['database']." DEFAULT CHARACTER SET utf8 DEFAULT COLLATE utf8_general_ci");
			$conn->query("USE ".$databaseConfig['database']);
	 ...
	 * Use with caution.
	 */
	public function dropDatabase($databaseConfig) {
		$conn = new MySQLi($databaseConfig['server'], $databaseConfig['username'], $databaseConfig['password']);
		$conn->query("DROP DATABASE ".$databaseConfig['database']);
		return true;
	}
}
