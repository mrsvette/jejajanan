...
<?php $form=$this->beginWidget('CActiveForm',array('id'=>'setting-form','htmlOptions'=>array('class'=>'form-horizontal'))); ?>
	...
	<h3 class="no-margin"><?php echo Yii::t('BannerModule.banner','Configure banner options');?></h3>
	<p class="mt10 mb10"><?php echo Yii::t('BannerModule.banner','Banner need several configuration. Please separated allowed file type with comas "," and fill file size in bytes.');?></p>

	<?php if($model['configs']): ?>
	<?php foreach($model['configs'] as $id=>$config):?>
	<div class="form-group row <?php if($config['hidden']):?>hide<?php endif;?>">
		<label class="col-md-3"><?php echo Yii::t('BannerModule.banner',$config['title']);?></label>
		<div class="col-md-4">
	...
	<div class="form-group buttons col-md-12">
		<?php 
		echo CHtml::ajaxSubmitButton(Yii::t('global', 'Save'),CHtml::normalizeUrl(array('/banner/bDefault/setting')),array('dataType'=>'json','success'=>'js:
				function(data){
