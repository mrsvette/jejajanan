...
class ETwigViewRenderer extends CApplicationComponent implements IViewRenderer
{
    ...
    function init()
    {
		...
		// add staticCall for accessing Class (usage: {{ call('myClass', 'mymethod', [optional params]) }})
		$this->_twig->addFunction('call', new Twig_Function_Function('ETwigStaticCall'));
	...
	public function renderText($context, $data, $return=true)
    {
        // current controller properties will be accessible as {{ this.property }}
        $data['this'] = $context;
		$app = Yii::app();
		...
		// add staticCall for accessing Class (usage: {{ call('myClass', 'mymethod', [optional params]) }})
		$this->_twig->addFunction('call', new Twig_Function_Function('ETwigStaticCall'));
        $template = $this->_twig->loadTemplate($context)->render($data);
...
function ETwigStaticCall($class, $function, $args = array())
{
